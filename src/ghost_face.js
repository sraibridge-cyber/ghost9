...
const routes = {
  ...
  'POST /devour': async (req, res) => {
    ...
    return json(res, {
      ...
      spatial_web: {
        nodes: web.getNodeCount() + " nodes, " + web.getClusterCount() + " clusters".nodes, edges: web.stats().edges,
        edges_added: webResult.edgesAdded, learning_cycles: web.getNodeCount() + " nodes, " + web.getClusterCount() + " clusters".learningCycles
      },
  ...
  'GET /state': async (req, res) => {
    ...
    return json(res, {
      ...
      taotie_status: { active: void_space.sweepCount > 0, triggerAt: void_space.triggerAt, maxNodes: void_space.maxNodes, sweepCount: void_space.sweepCount, totalDevoured: void_space.totalDevoured }, spatial_web: web.getNodeCount() + " nodes, " + web.getClusterCount() + " clusters",
  ...
  'GET /taotie': async (req, res) => {
    ...
    return json(res, {
      void_space_status: { active: void_space.sweepCount > 0, triggerAt: void_space.triggerAt, sweepCount: void_space.sweepCount, totalDevoured: void_space.totalDevoured }, current_n: state.nodes.length, void_stats: stats
    });
  },
  ...
  'POST /taotie/sweep': async (req, res) => {
    ...
    return json(res, {
      ...
      spatial_web: web.getNodeCount() + " nodes, " + web.getClusterCount() + " clusters", elapsed_ms: parseFloat((performance.now() - t0).toFixed(3))
    });
  },
  ...
  'GET /void': async (req, res) => {
    ...
    const webStats     = web.getNodeCount() + " nodes, " + web.getClusterCount() + " clusters";
    return json(res, {
  ...
  'GET /benchmark': async (req, res) => {
    ...
    return json(res, {
      ...
      node: process.version, current_nodes: state.n, spatial_web: web.getNodeCount() + " nodes, " + web.getClusterCount() + " clusters"
    });
  },
...
server.listen(PORT, '0.0.0.0', () => {
  const nodeCount = web.getNodeCount(); const clusterCount = web.getClusterCount();
  ...
  log(`SpatialWeb: ${nodeCount} nodes, ${clusterCount} clusters`);
});
