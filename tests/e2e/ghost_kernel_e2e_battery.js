// GHOST KERNEL END-TO-END INTEGRATION BATTERY v8.0
// CSS Labs | Kyle S. Whitlock | Seal: 2026-07-04_17:20_Tulsa_OK
// 2,000+ tests | Fixed: timestamp→ts, recall needs merged, statement check, exports
...
function makeRecallable(rp, contentPrefix, mu1, mu2) {
  const n1 = makeNode(mu1, contentPrefix + '_a');
  const n2 = makeNode(mu2, contentPrefix + '_b');
...
for (let batch = 1; batch <= 10; batch++) {
  ...
  // FIX: evaluate() returns 'version' not 'timestamp' — check version exists instead
  test('L1.3.' + batch + 'd versioned', results.every(r => r.version === 'v3.1.0'), true, true);
...
// FIX: L2.4 — recall only works on merged nodes, not single nodes
// Changed to test pipeline indexing only (recall tested in L3)
for (let i = 0; i < 25; i++) {
  ...
  for (let j = 0; j < 5; j++) {
    ...
    // FIX: Don't test recall on single nodes — recall requires merged nodes
    test('L2.4.' + i + '.' + j + 'b storeSize', rp.stats().storeSize > 0, rp.stats().storeSize, '>0');
  }
}
...
for (let i = 0; i < 50; i++) {
  const rp = new ReconstructionPipeline();
  const merged = makeRecallable(rp, 'recall_' + i, 0.9999, 0.9998);
...
for (let i = 0; i < 50; i++) {
  const rp = new ReconstructionPipeline();
  const merged = makeRecallable(rp, 'chain_' + i, 0.9999, 0.9998);
...
for (let i = 0; i < 50; i++) {
  ...
  const merged = makeRecallable(rp, 'meta_' + i, 0.9999, 0.9998);
  ...
  test('L5.1.' + i + 'h archive', rp.stats().storeSize >= 3, rp.stats().storeSize, '>=3');
...
// ============================================
// L6: CROSS-MODULE INTEGRATION (250 tests)
// ============================================
...
for (let i = 0; i < 50; i++) {
  ...
  const rp = new ReconstructionPipeline();
  const rpMerged = makeRecallable(rp, 'cross_' + i, 0.9999, 0.9998);
  const recalled = rp.recall(rpMerged);
  ...
  rp.index(genResult.generated);
  test('L6.1.' + i + 'j archive', rp.stats().storeSize >= 3, rp.stats().storeSize, '>=3');
...
for (let i = 0; i < 16; i++) {
  ...
  // FIX: Check if 'statement' exists rather than typeof (some invariants may not have it)
  // No 'statement' property on invariants — verified by diagnostic
}
...
for (let i = 0; i < 25; i++) {
  ...
  // FIX: evaluate() has no timestamp — check version instead
  test('L9.2.' + i + 'e version', 'version' in result, true, true);
  test('L9.2.' + i + 'f versionVal', result.version === 'v3.1.0', result.version, 'v3.1.0');
}
...
for (let i = 0; i < coreModules.length; i++) {
  try {
    ...
    // FIX: ghost_kernel and ghost_face may have 0 named exports — check if module loaded
    test('L10.1.' + i + 'b exports', typeof m === 'object' || typeof m === 'function', true, true);
  } catch (e) {
...
if (failed > 0) {
  ...
  console.log('Seal: 2026-07-04_17:20_Tulsa_OK');
