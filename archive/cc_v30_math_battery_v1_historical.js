// ============================================================
// CC v3.0 PURE MATH VALIDATION BATTERY
// CSS Labs | Kyle S. Whitlock | Seal: 2026-06-19_19:30_Tulsa_OK
...
// μ = exp((1/8) * Σ ln(si)) = geometric mean of 8 scores
function mu(scores) {
...
// Test 1.3: Hard-block — one domain at 0.0001
const hardBlock = [0.9997, 0.9997, 0.9997, 0.0001, 0.9997, 0.9997, 0.9997, 0.9997];
test('1.3 μ hard-block (one 0.0001)', mu(hardBlock) < 0.1, mu(hardBlock), '< 0.1');
...
// Test 1.6: Gate reachability — μ_max > τ
...
function scoreD5(text) {
    const lower = text.toLowerCase();
    if (/\bdo not\b/.test(lower) && /\bdo\b/.test(lower)) return 0.70;
...
test('6.2 D5 do not + do', scoreD5('do not do that') === 0.70, scoreD5('do not do that'), 0.70);
test('6.3 D5 do not + don\'t', scoreD5('do not don\'t worry') === 0.70, scoreD5('do not don\'t worry'), 0.70);
test('6.4 D5 only do not', scoreD5('do not worry') === 0.9997, scoreD5('do not worry'), 0.9997);
...
test('9.1 D8 empty memory', scoreD8('hello world testing today', []) === 0.9997, scoreD8('hello world testing today', []), 0.9997);
test('9.2 D8 <5 words', scoreD8('hi', []) === 0.0001, scoreD8('hi', []), 0.0001);
test('9.3 D8 near-duplicate', scoreD8('hello world testing today', ['hello world testing today']) === 0.0001, scoreD8('hello world testing today', ['hello world testing today']), 0.0001);
test('9.4 D8 high overlap', scoreD8('hello world testing today and more', ['hello world testing today']) === 0.9940, scoreD8('hello world testing today and more', ['hello world testing today']), 0.9940);
test('9.5 D8 moderate overlap', scoreD8('hello world new different words here', ['hello world testing today']) === 0.9990, scoreD8('hello world new different words here', ['hello world testing today']), 0.9990);
...
// 10.3: Hard-block any domain
const gate_hard = [0.0001, 0.9997, 0.9997, 0.9997, 0.9997, 0.9997, 0.9997, 0.9997];
test('10.3 Gate HARD-BLOCK (D1=0.0001)', mu(gate_hard) < 0.1, mu(gate_hard), '< 0.1');
...
// 11.2: Seven at ceiling, one at s_min
const seven_ceil = [0.9960, 0.9997, 0.9997, 0.9997, 0.9997, 0.9997, 0.9997, 0.9997];
testApprox('11.2 Seven ceiling, one s_min', mu(seven_ceil), 0.9995, 1e-4);
...
test('11.4 Empty text (D1+D2 hard-block)', mu(empty_text) < 0.1, mu(empty_text), '< 0.1');
...
// 12.2: Signal > Cognitive → P, Energy < Temporal → N
...
test('12.2 Mixed → PNNP', assignVertex(mixed1) === 'PNNP', assignVertex(mixed1), 'PNNP');
...
// 12.5: Verify 16 vertices exist
const vertices = new Set();
for (let a1 of [0.9997, 0.0001]) {
    for (let a2 of [0.9997, 0.0001]) {
        for (let a3 of [0.9997, 0.0001]) {
            for (let a4 of [0.9997, 0.0001]) {
                const d = {signal: a1, energy: a2, temporal: a3, spatial: a4, cognitive: a1, ethical: a3, declarative: a4, novelty: a2};
                vertices.add(assignVertex(d));
            }
        }
    }
}
test('12.6 All 16 vertices reachable', vertices.size === 16, vertices.size, 16);
